    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        ...
        for (y, row) in arr.iter().enumerate() {
            ...
            for (x, col) in row.chars().enumerate() {
                f.0[y].push(Octopus::new(col.to_digit(10).unwrap().try_into().unwrap()));
            }
        }
    ...
    if octo.value > 9 && !octo.flashed {
        ...
        for dy in -1i32..2 {
            for dx in -1i32..2 {
                ...
                if data.has(xx as usize, yy as usize) && !data.at(xx as usize, yy as usize).flashed
                {
