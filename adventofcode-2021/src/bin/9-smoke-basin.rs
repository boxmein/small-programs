...
impl Location {
    pub fn new(x: i32, y: i32, value: i32) -> Self {
        Self { x, y, value }
    }
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        ...
        for (y, row) in arr.iter().enumerate() {
            ...
            for (x, col) in row.chars().enumerate() {
                ...
                f.y_x.get_mut(&(y as i32)).unwrap().insert(
                    x as i32,
                    Location::new(
                        x as i32,
                        y as i32,
...
fn find_adjacencies(floor: &Floor, center: &Location) -> Vec<Location> {
...
fn find_local_minima(f: &Floor) -> Vec<Location> {
...
fn expand_basin(floor: &Floor, basin: &mut Basin, loc: &Location) {
...
fn find_basins(floor: &Floor, minima: &[Location]) -> Vec<Basin> {
...
9899965678"#;
    ...
    fn score_works() {
        ...
        assert_eq!(score(minima), 1 + 2 + 1 + 3 + 1 + 4 + 1 + 5 + 1 + 6 + 1 + 7);
    }
