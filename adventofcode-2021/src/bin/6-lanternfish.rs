...
impl Lanternfish {
    pub fn live_one_day(&mut self) -> bool {
        if self.0 == 0 {
            self.0 = 6;
            return true;
        }
...
fn main() {
    let mut lanternfish = commands(stdin().lock())
        .next()
        .expect("expected one line of input")
        .split(",")
        .map(|item| item.parse::<u8>().expect("not uint8"))
        .map(|item| Lanternfish(item))
        .collect::<Vec<Lanternfish>>();

    let mut new_lanternfish: Vec<Lanternfish> = vec![];

    for day in 0..DAYS {
        for fish in &mut lanternfish {
            // true if reproduces
            if fish.live_one_day() {
                new_lanternfish.push(Lanternfish(8));
            }
        }
        ...
        // move new to old
        lanternfish.append(&mut new_lanternfish);

        // println!("After {:>2} days: {}",
        //     day,
        //     lanternfish
        //         .iter()
        //         .map(|i| i.to_string())
