...
impl LineSegment {
    ...
    pub fn intersects(&self, other: &LineSegment) -> bool {
        ...
        let left0 = Mat2x2(a.x - c.x, b.x - c.x, a.y - c.y, b.y - c.y)
            .determinant()
            .sign();

        let right0 = Mat2x2(a.x - d.x, b.x - d.x, a.y - d.y, b.y - d.y)
            .determinant()
            .sign();

        let left1 = Mat2x2(c.x - a.x, d.x - a.x, c.y - a.y, d.y - a.y)
            .determinant()
            .sign();

        let right1 = Mat2x2(c.x - b.x, d.x - b.x, c.y - b.y, d.y - b.y)
            .determinant()
            .sign();
        ...
        for v1 in &self.vertices {
            for v2 in &self.vertices {
                if v1.intersects(&v2) {
                    ...
                    if !result.contains(&t1) && !result.contains(&t2) {
                        result.insert(t1);
                    }
                }
            }
...
fn main() {
    ...
    for command in commands(stdin().lock()) {
        ...
        map.add_vertex(LineSegment(first, last));
    }
    ...
    let window = video_subsystem
        .window("Hydrothermal Ventures", 1000, 1000)
        .position_centered()
        .build()
        .unwrap();
...
mod tests {
    ...
    fn determines_intersection_correctly() {
        // \/
        // /\
        let l1 = LineSegment(Point { x: -2, y: -2 }, Point { x: 2, y: 2 });
        let l2 = LineSegment(Point { x: -2, y: 2 }, Point { x: 2, y: -2 });
    ...
    fn determines_non_intersection_correctly() {
        ...
        // /  |
        let l1 = LineSegment(Point { x: -2, y: -2 }, Point { x: 2, y: 2 });
        let l2 = LineSegment(Point { x: 3, y: 2 }, Point { x: 3, y: -2 });
    ...
    fn shared_point_is_intersection() {
        ...
        // /  |
        let l1 = LineSegment(Point { x: -2, y: -2 }, Point { x: 2, y: 2 });
        let l2 = LineSegment(Point { x: 3, y: 2 }, Point { x: 2, y: 2 });
